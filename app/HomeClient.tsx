...
export default function HomeClient({ initialPrompts = [] }: { initialPrompts: Prompt[] }) {
    ...
    return (
        <div className="w-full min-h-screen bg-white dark:bg-[#0f1115] transition-colors duration-500">
            {/* 1. 搜索与筛选交互行 
                -mt-10 确保它漂浮在黑色 Hero 区域上方
                flex-nowrap 确保在任何屏幕下 Search 和 Filter 都在同一行
            */}
            <div className="relative -mt-10 px-6 z-50">
                <div className="max-w-6xl mx-auto flex flex-nowrap items-center gap-3">
                    {/* 搜索框：占据剩余所有空间 */}
                    <div className="flex-grow relative bg-white dark:bg-[#1a1c20] rounded-[22px] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] dark:shadow-[0_30px_70px_-10px_rgba(0,0,0,0.7)] border border-slate-100 dark:border-white/5 transition-all">
                        ...
                        <input
                            ...
                            className="w-full h-16 pl-16 pr-6 bg-transparent border-none text-base outline-none text-slate-800 dark:text-slate-100 placeholder:text-slate-400"
                        />
                    </div>
                    
                    {/* Filter 按钮：固定宽度，高度与搜索框严格对齐 */}
                    <button 
                        onClick={() => setIsFilterOpen(!isFilterOpen)}
                        className={`flex-shrink-0 h-16 px-8 rounded-[22px] flex items-center gap-3 font-bold text-sm transition-all border shadow-xl ${
                            isFilterOpen || selectedTags.length > 0
                            ? 'bg-[#3fc1c0] border-[#3fc1c0] text-white shadow-[#3fc1c0]/20' 
                            : 'bg-white dark:bg-[#1a1c20] text-slate-600 dark:text-slate-300 border-slate-100 dark:border-white/5 hover:border-[#3fc1c0]/50'
                        }`}
                    >
                ...
                {/* 2. 伸缩标签面板：适配参考图的扁平化风格 */}
                {isFilterOpen && (
                    <div className="max-w-6xl mx-auto mt-4 bg-white/95 dark:bg-[#1a1c20]/95 backdrop-blur-xl rounded-[28px] p-8 shadow-2xl border border-slate-100 dark:border-white/10 animate-in fade-in slide-in-from-top-2 transition-all">
